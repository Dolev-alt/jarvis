...
class NeuralSwitchboard:
    """The Multi-Provider Brain of JARVIS. Failover order: Gemini -> Groq."""
    ...
    def __init__(self, gemini_api_keys: str, gemini_model: str,
        ...
        print(f"[NEURAL SWITCHBOARD] Initialized. Gemini: {len(self.gemini_keys)} keys | Groq: {len(self.groq_keys)} keys")
    ...
    def send_gemini_stream(self, contents):
        """Stream Gemini response — yields text chunks as they arrive.
        Falls back to non-streaming on error (caller gets a single chunk)."""
        ...
        while attempts < len(self.gemini_keys):
            try:
                ...
                if "429" in str(e) or "RESOURCE_EXHAUSTED" in str(e):
                    print(f"[SWITCHBOARD] Gemini Stream Limit Hit. Rotating...")
                    self._rotate_gemini()
                    attempts += 1
                else:
                    print(f"[SWITCHBOARD] Gemini stream error: {e}, falling back to non-stream")
    ...
    def send_message(self, contents, uncensored=False, forced_provider=None, lang="en"):
        ...
        return type("Err", (), {"text": "Sir, all neural systems are currently unavailable."})()
